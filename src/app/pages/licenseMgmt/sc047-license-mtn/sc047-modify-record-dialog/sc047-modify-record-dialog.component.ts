...
import { OUInfo } from './../../../../core/model/ou-info';
import { VenCusInfo } from './../../../../core/model/ven-cus-info';
import { AuthApiService } from './../../../../core/services/auth-api.service';
import { ToastService } from './../../../../core/services/toast.service';

@Component({
  selector: 'app-sc047-modify-record-dialog',
  templateUrl: './sc047-modify-record-dialog.component.html',
  styleUrls: ['./sc047-modify-record-dialog.component.scss'],
})
export class Sc047ModifyRecordDialogComponent implements OnInit {
  ...
  onInitParams(): void {
    const getOUInfo$ = (ouCode: string) =>
      new Observable<OUInfo[]>((obs) => {
        this.authApiService
          .ouQueryByPrefix(ouCode)
          .pipe(take(1))
          .subscribe({
            next: (res) => {
              obs.next(res.ouList);
    ...
    if (this.settingParams && this.settingParams.data) {
      for (const key of Object.keys(this.formGroup.getRawValue())) {
        if (key.includes('Date')) {
          ...
          getOUInfo$(this.settingParams.data['oouCode']).subscribe((res) => {
            this.formGroup
              .get(key)
              .setValue(
                `${res[0].ouCode} - ${res[0].ouName} ( ${res[0].ouShortName} )`
              );
          });
        } else if (key === 'ouName') {
          getOUInfo$(this.settingParams.data['ouCode']).subscribe((res) => {
            this.formGroup
              .get(key)
              .setValue(
                `${res[0].ouCode} - ${res[0].ouName} ( ${res[0].ouShortName} )`
  ...
  onInitOptions(): void {
    ...
    this.formGroup.get('countryCode').disable({ onlySelf: true });
    getIELicenseRules$({
      ...
      this.formGroup.get('countryCode').enable({ onlySelf: true });
  ...
  onFormSubmit(): void {
    ...
    const oouName = this.oouKit
      ? this.oouKit.ouName
      : this.settingParams.data.oouName;

    const ouName = this.ouKit
      ? this.ouKit.ouName
      : this.settingParams.data.ouName;
    ...
    let vcNameE: string = !this.vcKit
      ? this.formGroup.get('vcNameE').value
        ...
        .subscribe({
          next: (res) => {
            ...
            if (res.code === ResponseCodeEnum.LICENSEMASTER_ALREADY_EXISTS) {
              ...
              this.toastService.success('Sc047LicenseMtn.Message.EditSuccess');
              this.onSubmit.emit();
          ...
          error: (err) => {
            this.loaderService.hide();
            console.error(err);
            this.toastService.error('Sc047LicenseMtn.Message.EditFailed');
          },
        });
    }
  }
