...
import { ItemInfo, LicenseItemInfo } from 'src/app/core/model/item-info';
...
@Component({
  selector: 'app-add-exp-multi-item',
  templateUrl: './add-exp-multi-item.component.html',
  styleUrls: ['./add-exp-multi-item.component.scss'],
})
export class AddExpMultiItemComponent implements OnInit, OnChanges {
  ...
  onDownloadSampleFileEvent(): void {
    this.commonApiService.downloadFile(1989);
  }
    ...
    const getMultiItemListByFile$ = (
      ...
      new Observable<any>((obs) => {
        this.licenseControlApiService
          .getEXPMultiItemListByFile(formNo, file)
          .pipe(takeLast(1))
          .subscribe({
            next: (res) => {
