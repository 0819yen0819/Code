...
@Component({
  selector: 'sov-history-log',
  templateUrl: './sov-history-log.component.html',
  styleUrls: ['./sov-history-log.component.scss'],
})
export class SovHistoryLogComponent implements OnInit, OnDestroy {
  @Input() formNo: string;
  ...
  private getHistory() {
    this.myFlowService.getFlowAuditLog(this.formNo,this.formTypeId).subscribe({
      next: (rsp) => {
        const historyStatus = ['Submitted', 'Approve', 'Reject', 'Rollback', 'Resolve', 'Cancel','Withdraw'];
