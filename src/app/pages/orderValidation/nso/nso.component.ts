...
import { NsoApiService } from './nso-api.service';
...
export class NsoComponent implements OnInit {
  ...
  headerSetting = [
    { col: "單號", content: "***NSO202305000254" },
    { col: "申請日期", content: "***2023-05-18" },
    { col: "申請人", content: "***153206 葉昕濠 Ray Yeh" }
  ]
  ...
  constructor(
    ...
    private nsoApiService: NsoApiService,
 ...
 */
  subscribeRoute() {
    this.activatedRoute.queryParams.subscribe(async (params) => {
      ...
      const formContent = await lastValueFrom(this.nsoApiService.getNSOChgForm(this.formNo)) // formContent
      ...
      Promise.all([
        ...
        lastValueFrom(this.nsoApiService.getFormTitle()), // formTitle
  ...
  getLineChange(e) { this.lineInfo = e; }
  ...
  getApproveDialogRes(e) {  // 取得審核事件
    ...
    lastValueFrom(this.nsoApiService.approveNewSoForm(model))
      .then(res => {
        this.storeBatchEditEntryService.postMessageToParent('Form Destroy');
  ...
  getWithdrawSubmit(comment) { //取得撤回事件
  ...
  private canApprove(e) {
    let errorMsg = '';
    const salesDeptIsEmpty = !this.deptInfo;
